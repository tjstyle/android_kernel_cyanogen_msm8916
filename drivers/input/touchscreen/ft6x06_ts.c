...
#define FT_CONFIG_UPGRADE_FIRMWARE 0

#define FT_IRQ_FLAG  0x2008

/*add by shenwenbin for ft_rw_iic_drv œÚµã apkÉýŒ¶¹ÌŒþÓÃ*/
#define FTS_CTL_IIC     //ft_rw_iic_drv¶ÁÐŽœÚµã.apkÉýŒ¶ºê
#ifdef FTS_CTL_IIC
#include "focaltech_ctl.h"
#endif

#define FTS_GESTRUE 1

#if FTS_GESTRUE
#include "ft_gesture_lib.h"

#define GTP_GESTURE_KEY_TAB  {KEY_O,KEY_W,KEY_M,KEY_E,KEY_C,KEY_UP,KEY_DOWN,KEY_LEFT,KEY_RIGHT}
static const u16 gesture_key_array[] = GTP_GESTURE_KEY_TAB;
#define GTP_MAX_GESTURE_KEY_NUM  (sizeof(gesture_key_array)/sizeof(gesture_key_array[0]))
...
int gestrue_id = 0;
#endif
#if FT_CONFIG_UPGRADE_FIRMWARE
typedef unsigned char         FTS_BYTE;     //8 bit
#define FTS_NULL                0x0

static unsigned char CTPM_FW[]=
{
#include "Ft6x06_Ver0d_20140722_app.i"
};
#endif
...
static u32 enable_gpio = -1;
static u32 enable_gpio_flags;
...
static int tp_prox_sensor_enable(int enable);

static struct sensors_classdev sensors_proximity_cdev = {
	.name = "ft6x06-proximity",
	.vendor = "ft6x06",
	.version = 1,
...
#if FTS_GESTRUE
static uint fts_enter_gestrue(struct ft5x06_ts_data *data)
{
    s8 ret = -1;
    s8 retry = 0;
    ...
    while(retry++ < 5)
    {
		...
		ret = ft5x06_i2c_write(data->client, auc_i2c_write_buf, 2);
        if (ret > 0)
        {
...
static int gesture_ft5x06_read_and_report_Touchdata(struct ft5x06_ts_data *data)
{
	...
	ret = ft5x06_i2c_read(data->client, buf, 1, buf, FTS_GESTRUE_POINTS_HEADER);
	if (ret < 0)
	{
	...
	if (0x24 == buf[0])
	{
		printk(KERN_INFO"liangbin.he test  buf[0] = %d\n",buf[0]);
		gestrue_id = 0x24;
	...
	/*	Byte0 	GESTURE ID
			...
			Byte2--Byte7
			Byte8--Coordinate */
	ret = ft5x06_i2c_read(data->client, buf, 1, buf, (pointnum * 4 + 2 + 6));
	/*if limit everytime only read 8 bytes*/
	//ret = ft5x0x_i2c_Read(data->client, buf, 1, buf, (8));
	//ret = ft5x0x_i2c_Read(data->client, NULL, 0, (buf+8), (8));	
	...
	printk(KERN_INFO"liangbin.he  gestrue_id = 0x%x\n",gestrue_id);	
	...
	{
		fts_enter_gestrue(data);
	}
	
	return 0;
}
#endif
static int ft5x06_read_and_report_Touchdata(struct ft5x06_ts_data *data)
{
	...
	if (tp_prox_sensor_opened == 1)
	{
		...
		if(!(state&0x01))// if the psensor is not enable, enable here
		{
...
static ssize_t tp_ps_enable_set(struct sensors_classdev *sensors_cdev,
						unsigned int enabled)
{
	struct ft5x06_ts_data *ps_data = container_of(sensors_cdev,
						struct ft5x06_ts_data, ps_cdev);
	int err;
	struct i2c_client *client = ps_data->client;
	struct input_dev *input_dev = ps_data->input_dev;

	mutex_lock(&input_dev->mutex);
	disable_irq(client->irq);

	mutex_lock(&tp_prox_sensor_mutex);
	err=tp_prox_sensor_enable((int)enabled);
	
	TPD_DMESG("tp_ps_enable_set enable=%d\n", enabled);
	if(enabled){
	#ifdef SENSOR_PROX_TP_USE_WAKELOCK
					...
					const char *buf, size_t count)
{
	...
	//struct ft5x06_ts_data *prox = i2c_get_clientdata(client);
...
{
	struct i2c_client *client = to_i2c_client(dev);
	struct ft5x06_ts_data *ps = i2c_get_clientdata(client);
...
static void __init tp_prox_init_input_device(struct ft5x06_ts_data *prox,
					      struct input_dev *input_dev)
{
...
{
	struct ft5x06_ts_data *prox = dev->private;
...
static int __init tp_prox_setup_polled_device(struct ft5x06_ts_data *ps)
{
	int err;
...
static void tp_prox_teardown_polled_device(struct ft5x06_ts_data *ps)
{
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	...
	if (data->suspended)
	{
		rc = gesture_ft5x06_read_and_report_Touchdata(data);
		if(rc)
			return IRQ_HANDLED;
	}
	else
#endif	
	{
		rc = ft5x06_read_and_report_Touchdata(data);
		if(rc)
			return IRQ_HANDLED;
	}
...
//	printk("ft5x06_power_on on\n");
	if (gpio_is_valid(enable_gpio)) {
		rc = gpio_direction_output(enable_gpio, 1);
		if (rc) {
			dev_err(&data->client->dev,
				"power_on, gpio_direction_output enable_gpio failed rc=%d\n", rc);
			return rc;
		}
	}
...
power_off:
//	printk("ft5x06_power_on off\n");
	if (gpio_is_valid(enable_gpio)) {
		rc = gpio_direction_output(enable_gpio, 0);
		if (rc) {
			dev_err(&data->client->dev,
				"power_off, gpio_direction_output enable_gpio failed rc=%d\n", rc);
			return rc;
		}
	}
			...
			struct ft5x06_ts_platform_data *pdata)
{
	...
	enable_gpio = of_get_named_gpio_flags(np, "focaltech,enable-gpio",
				0, &enable_gpio_flags);
	if (enable_gpio < 0)
	{
		dev_err(dev, "enable_gpio is error\n");
		return enable_gpio;
	}
	else
		printk("enable_gpio is %d\n", enable_gpio);
			   ...
			   const struct i2c_device_id *id)
{
	...
	for (index = 0; index < GTP_MAX_GESTURE_KEY_NUM; index++)
	{
		input_set_capability(input_dev, EV_KEY, gesture_key_array[index]);
	}
	...
	if (gpio_is_valid(enable_gpio)) {
		err = gpio_request(enable_gpio, "ft5x06_enable_gpio");
		if (err) {
			dev_err(&client->dev, "enable_gpio gpio request failed");
			goto free_inputdev;
		}
		err = gpio_direction_output(enable_gpio, 0);
		if (err) {
			dev_err(&client->dev,
				"set_direction for enable_gpio failed\n");
			goto free_irq_gpio;
		}
	}	
	if (pdata->power_init) {
	...
	err = ft5x06_i2c_read(client, &reg_addr, 1, &reg_value, 1);	//read firmware version
	if (err < 0) {
		dev_err(&client->dev, "version read failed");
		goto free_reset_gpio;
	}
	...
	dev_info(&client->dev, "firmware version = 0x%x\n", reg_value);
     ...
     if(reg_value<0x0d)
      {
	     printk("----test start update TP  frameware!\n");

	     msleep(50);  //must add or "Step 3:check READ-ID" will return error 
	   pbt_buf = CTPM_FW;
 	  /*call the upgrade function*/
	  err = ft5x06_fw_upgrade_start(client, pbt_buf, sizeof(CTPM_FW));
...
//	       fts_ctpm_auto_clb();  //start auto CLB
	...
	err = request_threaded_irq(client->irq, NULL,
				   ft5x06_ts_interrupt ,FT_IRQ_FLAG,
...
//	ft5x06_update_fw_ver(data);
	...
	tp_prox_setup_polled_device(data);
	err = sysfs_create_group(&data->input_poll_dev->input->dev.kobj, &tp_prox_attribute_group);
	if (err) {
		TPD_DMESG( "sysfs create failed: %d\n", err);
	}

	data->ps_cdev = sensors_proximity_cdev;
	data->ps_cdev.sensors_enable = tp_ps_enable_set;
...
static int ft5x06_ts_remove(struct i2c_client *client)
{
    ...
    tp_prox_teardown_polled_device(data);
#ifdef SENSOR_PROX_TP_USE_WAKELOCK
